...
def get_characters():
    dicc_pj_recu = {}  # Inicializamos diccionario

    # Ejecutar consulta
    cursor.execute("SELECT * FROM characters")
    pj_recu = cursor.fetchall()  # Esto devuelve lista de diccionarios si cursor tiene dictionary=True

    # Preparar datos para imprimir
    datos = ""
    for pj in pj_recu:
        # Cambiamos nombres de columnas a los correctos
        datos += "{:<5}{:<45}\n".format(pj["id_character"], pj["name"])
        dicc_pj_recu[pj["id_character"]] = pj["name"]

    # Imprimir resultados
    print("ID   Character Name".center(50, "="))
    print(datos)
    print("Diccionario:", dicc_pj_recu)
...
from db import get_connection  # Asegúrate de tener la función para la conexión
def getTable(query):
    """
    ...
    cursor.execute(query)
    rows = cursor.fetchall()
    headers = [i[0] for i in cursor.description]  # Obtener nombres de columnas

    result = [tuple(headers)]  # Primer elemento: cabecera
    for row in rows:
