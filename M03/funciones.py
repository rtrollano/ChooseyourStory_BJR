...
def get_adventures_with_chars():
    ...
    cursor.execute("SELECT id_adventure, name, description FROM adventures")
    aventuras_db = cursor.fetchall()

    adventures_dict = {}
    for adv in aventuras_db:
        adventures_dict[adv["id_adventure"]] = {
            "Name": adv["name"],
...
def build_answers_by_step_adventure(idAdventure):
    ...
    # Respuestas base
    cursor.execute("""
        SELECT id_answer, id_step, description, resolution_answer, next_step
        FROM answers
        WHERE id_adventure = %s
    """, (idAdventure,))
    answers = cursor.fetchall()

    # Estados afectados por respuesta
    cursor.execute("""
        SELECT ase.id_answer,
               gsm.name AS state_name,
               ase.value
        FROM answer_state_effects ase
        JOIN game_states_master gsm ON gsm.id_state = ase.id_state
        WHERE ase.id_adventure = %s
    """, (idAdventure,))
